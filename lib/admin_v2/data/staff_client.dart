...
const _staffTokenKey = 'staff_access_v2';

/// Fresh staff HTTP client for admin_v2 (does not import old admin/).
class StaffClient {
  StaffClient() {
  ...
  /// Impersonation subject JWT — used only by [getAsSubject].
  String? subjectToken;
  ...
  /// GET with the impersonation [subjectToken] as Authorization, then restore staff token.
  Future<Map<String, dynamic>> getAsSubject(String path, {Map<String, dynamic>? query}) async {
    final subject = subjectToken;
    if (subject == null || subject.isEmpty) {
      return get(path, query: query);
    }
    final staff = token ?? readToken();
    try {
      token = subject;
  ...
  Future<Uint8List> getBytes(String path, {Map<String, dynamic>? query}) async {
    try {
      ...
      final data = r.data;
      if (data == null || data.isEmpty) throw ApiException(r.statusCode ?? 0, 'empty export');
